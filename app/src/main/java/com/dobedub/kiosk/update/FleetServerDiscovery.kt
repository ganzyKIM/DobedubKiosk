 ...
 * 함대 서버를 자동으로 찾는다.
 *
 * v2.5부터는 후보 URL(저장된 주소·빌드 기본값=NetBird 고정 주소)의 `/health` 를 먼저
 * 두드린다(`discoverSmart`) — NetBird 에 등록된 태블릿은 어느 망에 있든 여기서 잡힌다.
 * 후보가 전부 침묵할 때만 예전 방식으로 폴백: 기기 자신의 IP에서 /24 서브넷을 뽑아
 * 전 대역에 `GET /health` 를 병렬로 던지고 `ok` 로 답하는 호스트를 잡는다(보통 2~3초,
 * 같은 공유기 안의 임시/개발 서버용).
 */
