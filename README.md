...
기획 배경/디자인 근거/리스크는 [기획문서.md](기획문서.md) 참고. 현재 버전 **2.5.6**
(`versionCode=33`). 버전별 변경 이력은 개발_노트.md "현재 상태" 참고.
프로젝트 전체 컨텍스트·최근 변경사항은 [개발_노트.md](개발_노트.md) 참고.
...
맥에서 개발할 때의 도구 설치·스크립트 대응표는 **[DEV_SETUP.md](DEV_SETUP.md) §10**.
...
(마이보이스 카운트다운 숫자가 중심에서 밀림). 상세 원인·CDP 실측값은 `개발_노트.md`와 해당
파일 상단 주석 참고.

**건드리면 안 되는 것**: 리더 폭(460px)을 CSS로 넓히려는 시도는 이미 해봤고 실패했다 —
사이트가 스크롤-오디오 싱크를 460 기준으로 하드코딩해서 폭만 넓히면 싱크가 깨진다.
근본 해결은 사이트 쪽에서 460 캡을 제거해야 한다(자세한 내용은 `개발_노트.md` "건드리면
안 되는 것" 참조).
