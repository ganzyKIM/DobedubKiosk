...
두 가지 "만료"가 있고 **영향 범위가 완전히 다르다**. 헷갈리면 "어느 날 함대가 통째로
끊긴다"는 잘못된 공포를 갖거나, 반대로 진짜 위험을 놓친다.

| 대상 | 등록 방식 | 만료 | 조치 |
|---|---|---|---|
| **함대 서버 PC** | 사람 계정 SSO | **24시간 뒤 로그인 풀림** (실측: 맥에서 발생) | 콘솔에서 그 피어의 **Login Expiration 끄기**. 안 하면 매일 `netbird up` 재실행 필요 |
| **태블릿** | Setup Key | 안 풀림 | 없음 |

- **Setup Key 만료는 등록 시점에만 의미가 있다.** 이미 등록된 피어는 자기 신분으로
  동작하므로 키가 만료돼도 무관하다. 만료되면 **새 기기 등록만** 막힌다.
- 실측 근거: 개발 태블릿을 2026-08-15 에 setup key 로 등록 → **2026-08-31 까지 16일간
  재인증 없이 연결 유지**(tun0 100.x, 피어 Connected). 같은 기간 맥(SSO)은 24시간 만에 풀렸다.
- 회사 운영 PC 로 넘길 때 이 설정을 빠뜨리면 **다음 날 원격 관리가 통째로 죽는다** —
  `회사_인수인계_가이드.md` 2-4 ②, 2-5-1 에 명시.
...
## 현재 상태 (2026-09-02) — v2.5.6 / `versionCode=33`
...
- v2.2 체크인 주기 30분 → **10분**. 실측 비용은 요청 2.1KB/응답 0.5KB/왕복 42ms 라 기기·
  ...
  - ⚠ 화면 꺼진 doze 는 **대용량 다운로드도 중간에 끊는다**(실측 2026-08-22 밤: 64MB APK 가
    45% 등에서 반복 사망 → 강제 업데이트가 아침까지 안 끝남). 밤에 급히 밀어야 하면
    **원격 재부팅을 대기열에 함께 얹을 것** — 재부팅 직후는 화면이 켜져 doze 없는 창이
    열리고, 그 안에서 다운로드가 완주한다(부팅 후 체크인 8초 실측).
...
- v2.5.6 **위치가 기기를 따라오지 않던 문제** — 좌표를 못 잡는 게 아니라 **첫 장소에서
  얼어붙는** 것이 본질이었다. 실측: 집에서 주운 좌표(±100m)가 하루 뒤 외출 후에도 그대로.
  - 원인 3중: ① 납품 기기는 NLP 가 꺼져 `network` provider 미등록이라 요청이 GPS 로만
    나가고, 실내에서는 3분 예산을 쓰고 포기한다 ② 그 ±100m 좌표는 사실 **passive
    provider** 로 남의 앱이 받아둔 값을 주운 것이라 언제 갱신될지 예측 불가
    ③ 서버는 좌표를 `COALESCE` 로 보존하므로 옛 값이 영구히 남는다(설치 위치를 잃지
    않으려는 의도적 설계).
  - 대책은 좌표를 고치는 게 아니라 **이동하면 반드시 바뀌는 신호를 추가**하는 것:
    `PublicIpHelper` 가 공인 IP 를 알아내 체크인에 싣는다(`publicIp` → `devices.public_ip`).
    **서버는 이 값을 스스로 알 수 없다** — 체크인이 NetBird 를 지나와 `req.ip` 가 100.x
    오버레이 주소이기 때문. 기기가 직접 보고해야 하는 이유가 이것이다.
  - `public_ip` 는 `COALESCE` 하지 않는다. 옛 IP 를 남기면 이동 감지라는 존재 이유가 사라진다.
  - 대시보드: 기기 셀에 공인 IP(클릭 시 ipinfo.io 조회 — 서버에 외부 의존성을 두지 않는다),
    지도 링크 옆에 **좌표 나이**("23시간 전"). 나이 표시가 없으면 운영자가 묵은 좌표를
    현위치로 읽고 "엉뚱한 도서관에 있다"고 오해한다.
  - 체크인을 지연시키지 않는다 — 캐시값 즉시 사용 + 백그라운드 갱신(30분), `inFlight` 가드.
    첫 체크인은 값이 비는 것이 정상이고 다음 체크인부터 실린다.
  - 실측: 핫스팟에서 `175.223.10.33` 보고, 맥의 공인 IP 와 일치 확인. 서버 테스트 19개.
