...
| `release/lan-v2.4` (태그 `v2.4-lan`) | **회사 납품용 중간 저장본 — `versionCode=25`.** v2.3.2 에서 갈라 동영상 플레이어 아동용 UI만 얹었고, **NetBird 계열은 전부 제외**. 같은 와이파이 안에서만 관리하는 판. 상세는 그 브랜치의 `납품_LAN버전.md` |

납품선에 패치가 필요하면 `versionCode` 는 **개발선 최고값보다 큰 수**를 쓴다(현재 29 → 30부터).
