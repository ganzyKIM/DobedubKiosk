...
> ⚠ **이 브랜치는 납품용 중간 저장본(`release/lan-v2.4`)이다.** 같은 와이파이 안에서만
> 관리하는 판이며, 원격 관리(NetBird)는 의도적으로 들어 있지 않다. 범위·재빌드 절차는
> `납품_LAN버전.md`. 최신 개발은 `mac-dev` 브랜치에서 이어진다.
...
| `납품_LAN버전.md` | **이 브랜치가 무엇인지 — 납품 범위·제외 항목·재빌드 절차** |
| `server/start-tunnel.ps1` / `.sh` | 공인 HTTPS 원격 접속 경로(Cloudflare Quick Tunnel) 실행 스크립트 |
| `기획문서.md` | 최초 기획 배경 |

## 현재 상태 (2026-08-18) — v2.4.2 / `versionCode=27`

`versionCode`는 절대 되돌리지 말 것(업데이트 트리거 기준). versionName은 v1.0에서 한 번
리셋했고 이후 계속 올라간다.

- **v2.4.2** 마이보이스 재생 끊김 수정 + 더빙 마이크 재설계.
  - `mediaPlaybackRequiresUserGesture = false` — 위 "재생이 서너 개만 되고 끊긴다" 참조.
    도서관 접수 건("내 목소리만 들린다")의 실제 원인이었다.
  - 마이크 체인 재설계 — 위 "마이크가 작고 지직" 참조. 증폭을 WebRTC AGC에 맡기고
    앱 게인은 +4dB 로 최소화, 컴프레서 제거, `noiseSuppression` 끔.
  - 실기기(HA1EKRNR) 재생·녹음 확인 완료. 회사 태블릿 4대는 code 25 → 27 로 배포 대상.
  - ⚠ 중간에 code 26(2.4.1)을 이 기기에만 설치했었다. 서버에 올라간 적은 없다.
